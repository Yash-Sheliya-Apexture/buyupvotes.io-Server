...
//     process.exit(1); // Exit if JWT refresh secret is missing
//   }
//   if (!process.env.SMTP_HOST || !process.env.SMTP_PORT || !process.env.SMTP_USER || !process.env.SMTP_PASS) {
//     console.error('FATAL ERROR: SMTP settings are not defined in environment variables.');
//     process.exit(1); // Exit if SMTP settings are missing
//   }
// };
...
//     // Respond with successful login, including tokens and user info
//     res.status(200).json({
//       code: "SUCCESS",
//       message: "Login successful.",
//       user: {
...
// export const getUserData = async (req, res) => {
//   try {
//     // Use the user data from the auth middleware
//     const user = req.user;
...
//     if (firstName && firstName.length > 50) {  // Example max length
...
//     });
//   } catch (err) {
//     console.error("Error updating user data:", err);  // Log the error
...
//             return res.status(400).json({ code: "INVALID_FIRST_NAME", message: "First name must be between 2 and 50 characters." });
//         }

//         if (!validator.isLength(lastName, { min: 2, max: 50 })) {
//             return res.status(400).json({ code: "INVALID_LAST_NAME", message: "Last name must be between 2 and 50 characters." });
//         }
...
//             return res.status(400).json({ code: "WEAK_PASSWORD", message: "Password must be at least 8 characters long." });
//         }
...
//         // Hash password and create user
//         const hashedPassword = await bcrypt.hash(password, 10); // **HASHING HERE** - salt rounds = 10
...
// // Login Controller (Modified - Cleaned Up)
// export const login = async (req, res) => {
//     try {
//         const { email: rawEmail, password: rawPassword } = req.body;

//         // Normalize and trim inputs consistently
//         const email = rawEmail.trim().toLowerCase();
//         const password = rawPassword.trim();

//         // Log the trimmed values for debugging (keep email for tracking attempts)
...
//             return res.status(400).json({ code: "INVALID_CREDENTIALS", message: "Invalid email." });
//         }

//         // Password comparison with trimmed input
//         const isMatch = await bcrypt.compare(password, user.password);

//         console.log("bcrypt.compare result:", isMatch); // Keep this for debugging
//         if (!isMatch) {
//             console.log(`Failed login attempt for user: ${email}`);
//             return res.status(400).json({ code: "INVALID_CREDENTIALS", message: "Invalid password." });
//         }
...
//         if (!refreshToken) {
//             return res.status(400).json({ code: 'NO_REFRESH_TOKEN', message: 'Refresh token is required.' });
//         }

//         const user = await User.findOne({ refreshToken });

//         if (!user) {
//             return res.status(400).json({ code: 'INVALID_REFRESH_TOKEN', message: 'Invalid refresh token.' });
//         }
...
//         res.status(200).json({ code: 'LOGOUT_SUCCESS', message: 'Logged out successfully.' });
//     } catch (err) {
...
const checkEnvVariables = () => {
    ...
    if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET) {
        console.error('FATAL ERROR: Google OAuth settings are not defined in environment variables.');
        process.exit(1); // Exit if Google OAuth settings are missing
    }
};
...
// Google Login Callback (Authorization Code Flow)
export const googleLoginCallback = async (req, res) => {
    try {
        console.log('Request Body for Google Login (Auth Code Flow):', req.body); // Debugging log

        const { code } = req.body;

        if (!code) {
            return res.status(400).json({ code: "MISSING_CODE", message: "Authorization code is required." });
        }

        const tokenResult = await googleClient.getToken(code);
        ...
        const { email_verified, email, name, given_name, family_name } = googleResponse.getPayload();

        if (!email_verified) {
            return res.status(400).json({ code: "EMAIL_NOT_VERIFIED", message: "Google email not verified." });
        }
        ...
        if (!user) {
            ...
            const newUser = new User({
                ...
                password: bcrypt.hashSync(Math.random().toString(36).substring(7), 10), // Random password
            });
        ...
        console.error("Google login error (Auth Code Flow):", error);
        res.status(500).json({ code: "GOOGLE_LOGIN_FAILED", message: "Google login failed.", error: error.message });
    }
};
...
export const adminLogin = async (req, res) => {
    try {
        ...
        // Check if admin user exists (adjust query as needed to target admin users)
        const adminUser = await User.findOne({ email: normalizedEmail, role: 'admin' });
        if (!adminUser) {
            return res.status(400).json({ code: "INVALID_EMAIL", message: "Invalid email or password for admin." });
        }

        // Verify password
        const isMatch = await bcrypt.compare(password, adminUser.password);
        if (!isMatch) {
            return res.status(400).json({ code: "INVALID_PASSWORD", message: "Invalid email or password for admin." });
        }
        ...
        // Respond with successful admin login, including tokens and user info
        res.status(200).json({
            code: "SUCCESS",
        ...
        console.error(err);
        res.status(500).json({ code: "SERVER_ERROR", message: "Admin login failed." });
    }
};